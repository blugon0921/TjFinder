...
object TJApi {
    private const val searchUrl = "https://www.tjmedia.com/tjsong/song_search_list.asp"
    private const val popularUrl = "https://www.tjmedia.com/tjsong/song_monthPopular.asp"
    private const val monthNewUrl = "https://www.tjmedia.com/tjsong/song_monthNew.asp"
    ...
    fun search(keyword: String, type: StringType, match: Boolean = false): List<Song> {
        val jsoup = Jsoup.connect("$searchUrl?strType=${type.code}&strText=$keyword&strSize0${type.sizeCode}=100&strCond=${if(match) 1 else 0}")
        ...
        repeat((document.select("#BoardType1 > table > tbody").first()?.childrenSize() ?: return listOf()) -1) {
            ...
            songs.add(Song(
                ...
                elements.children().getOrNull(1)?.children()?.getOrNull(1)?.classNames()?.contains("mr_icon")?: false, //isMR
            ))
        }
        return songs
    }
    ...
    fun monthPopular(type: SongType = SongType.K_POP): List<Top100Song> {
        val jsoup = Jsoup.connect("$popularUrl?strType=${type.code}")
        val document = jsoup.get()
    ...
    fun monthNew(): List<Song> {
        val document = Jsoup.connect(monthNewUrl).get()
        val songs = ArrayList<Song>()
        repeat((document.select("#BoardType1 > table > tbody").first()?.childrenSize() ?: return listOf()) -1) {
            val song = document.select("#BoardType1 > table > tbody > tr:nth-child(${it + 2})").first()?: return@repeat
            val elements = song.allElements.first()?: return@repeat
