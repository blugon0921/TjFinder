...
fun PlaylistTitle(
    ...
    if(withId) DoubleText(
        first = {
            ...
            it.style = PretendardTextStyle(fontSize = fontSize, fontWeight = FontWeight.Bold)
        },
        second = {
            it.text = "#${playlist.id}"
            it.style = PretendardTextStyle(color = Color.Gray, fontSize = fontSize-5)
        },
        ...
        Row( //스크롤 애니메이션 블러
            ...
            Row( //스크롤 애니메이션
                ...
                Text(buildAnnotatedString {
                    withStyle(style = PretendardTextStyle(fontSize = fontSize)) { append(playlist.title) } //플레이리스트 Title
                    withStyle(style = PretendardTextStyle(
                        color = Color.Gray,
                        fontSize = fontSize-5,
